...
class ShortLink {
    ...
    async linkpoi(url) {
        try {
            ...
            return result.shorturl.replace("/", "/");
        } catch (error) {
            console.error(error);
            return null;
        }
    ...
    async kutt(url) {
        try {
            const config = {
                headers: {
                    "X-API-KEY": "VcMiC4tZGdD1Lgu1KTiYfSNrs3Q_K3TMdVuSnStl" || "WHGqtwXmE4ubZ7dBkOSTLNFhrfiIr_gcJKG1gFbo" || "",
                    "Content-Type": "application/json",
                },
            };
            const jsonBody = {
    ...
    async multishort(url) {
        try {
            ...
            const result = await response.json();
            const linkList = Object.entries(result).map(([name, link]) => ({
                nama: name,
                link,
            }));
            const Liks = linkList.map((v, index) => "  ○ " + v.link).join("\n");
            return `${Liks}`;
        } catch (error) {
            console.error(error);
            return null;
    ...
    async shrtco(url) {
        try {
            ...
            const result = await response.json();
            const linkArray = Object.values(result.result).filter(value => !value.startsWith("https"));
            const ShrArr = linkArray.map((v, index) => "  ○ " + v).join("\n");
            return `${ShrArr}`;
        } catch (error) {
            console.error(error);
            return null;
    ...
    async dxyz(url) {
        try {
            const result = await (await fetch('https://d.131213.xyz/create', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    async shorturl(url) {
        try {
            ...
            return responseBody.split('id="shortenurl" type="text" value="')[1].split('"')[0];
        } catch (error) {
            console.error(error);
            return null;
        }
    ...
    async ssur(url) {
        try {
            ...
            const shortenerUrl = `https://ssur.cc/api.php?appkey=${randomKey}&format=json&longurl=${url}`;
            const response = await fetch(shortenerUrl);
            const shortedUrl = await response.json();
            return shortedUrl.ae_url;
        } catch (error) {
            console.error(error);
            return null;
        }
    ...
    async adfly(url) {
        try {
            const keys = ["9a283dafe534bddccc556153a37ed678",
                "d8a2283a6bbafbe31b442776fdc108ab",
                "9f7a757edc77cda2a8394aa6088c5c1a",
                "7efadeae65372322ee0205a4d9e2616a"
            ];
            const randomKey = keys[Math.floor(Math.random() * keys.length)];
            const shortenerUrl = `https://api.adf.ly/api.php?key=${randomKey}&uid=343503&advert_type=int&domain=adf.ly&url=${url}`;
            const response = await fetch(shortenerUrl);
