...
export default async function (m, conn = { user: {} }) {
  ...
  if (m.sender) {
    // Output Message Info by jhnspntx
    ...
    console.log(`   ${chalk.cyan('» Sender Name')}: ${conn.getName(m.sender) || 'N/A'}`);
    console.log(`   ${chalk.cyan('» Chat ID')}: ${m.chat.split('@')[0]}`);
    console.log(`   ${chalk.cyan('» Chat Name')}: ${chat || 'N/A'}`);
    console.log(`   ${chalk.cyan('» Author By')}: Jhnspntx`);
    console.log(chalk.cyan('───────────────────────────────────⳹\n'));
  }

  // Output text message with formatting
  if (typeof m?.text === 'string' && m.text) {
    console.log(chalk.cyan(`────────────『 ${chalk.redBright('ᶜᴼᴹᴹᴬᴺᴰ/ᶜʰᵃᵗ')} 』────────────`));
    ...
    console.log((m.error != null ? `🚨 ${chalk.red(truncatedLog)}` : (m.isCommand ? `➭ ${chalk.yellow(truncatedLog)}` : truncatedLog)));
    console.log(chalk.cyan('───────────────────────────────────⳹\n'));
  }
  ...
  if (m.msg) {
    ...
    if (/document/i.test(attachmentType)) {
      console.log(chalk.cyan(`────────────『 ${chalk.redBright('ᴰᵒᶜᵘᵐᵉⁿᵗ')} 』────────────`));
...
console.log(chalk.redBright(`» 🎵 Attached Audio: ${m.msg.ptt ? '(PTT)' : '(Audio)'} - Duration: ${formattedDuration}`));
   ...
   console.log(chalk.cyan(`────────────『 ${chalk.redBright('ᴵᴹᴬᴳᴱ')} 』────────────`));
  ...
  if (m.sender) {
    console.log(chalk.cyan(`────────────『 ${chalk.redBright('ˢᴱᴺᴰᴱᴿ ᴵᴺᶠᴼ')} 』────────────`));
