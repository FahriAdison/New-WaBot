...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	text.match(URL_REGEX) ?
		await conn.sendMessage(m.chat, { [mime.split('/')[0]]: { url: res }, caption: `Succes Download` }, { quoted: m }) :
	await conn.sendFile(m.chat, res, 'pinterest.jpg', `Result From: ${text.capitalize()}`, m)
}
