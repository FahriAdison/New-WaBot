export async function before(m) {
    try {
        ...
        if (!(id in this.bomb) && m.quoted && /kotak/i.test(m.quoted.text)) return this.reply(m.chat, `🚩Sesi telah berakhir, silahkan kirim _.bomb_ untuk membuat sesi baru.`, m)
        ...
        if ((id in this.bomb) && !isNaN(body)) {
            ...
            if (json.emot == '💥') {
                ...
                return this.sendFile(m.chat, 'https://telegra.ph/file/287cbe90fe5263682121d.jpg', '', teks, m).then(() => {
                ...
                if (open >= 8) {
                    ...
                    return this.sendFile(m.chat, 'https://telegra.ph/file/308a4f10cc4576a90b4a0.jpg', '', teks, m).then(() => {
                        users.exp += reward
                        clearTimeout(this.bomb[id][2])
                        delete this.bomb[id]
                    })
