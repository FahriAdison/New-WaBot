...
let handler = async (m, { conn, text, args, usedPrefix: _p, command: cmd }) => {
	...
	if (text && /soundcloud\.com\/[^/]+\//.test(args[0])) {
		...
		let msg = /^(?:-|--)doc$/i.test(args[1]) ? await conn.sendFile(m.chat, data.thumbnail, '', txt, m) : await conn.sendMessageModify(m.chat, txt, m, {
title: wait,
ads: true, 
largeThumb: true,
