...
//PEMBATAS
let penumpan = ['mas mas','bapak bapak','cewe sma','bocil kematian','emak emak']
...
//KARENA LIST BUTTON GA WORK JADI PAKE PESAN AJA
let text = `⎔───「 ${command} 」───⎔
...
if (new Date - users.lastkerja < cooldown)  throw `Kamu sudah bekerja\nSaatnya istirahat selama\n${((users.lastkerja + cooldown) - new Date()).toTimeString()}`
...
if (new Date - users.lastkerja < cooldown)  throw `Kamu sudah bekerja,Saatnya istirahat selama\n${((users.lastkerja + cooldown) - new Date()).toTimeString()}`
